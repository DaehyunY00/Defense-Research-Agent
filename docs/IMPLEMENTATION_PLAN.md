...
- 네 번째 교차 검토 보완(2026-08-10): 표지에 발행일이 없는 Brief와 본문에
  ...
  test로 고정했다.
- 다섯 번째 교차 검토 보완(2026-08-10): 입력은 발행 근거를 포함한
  `PublicationPage` 줄과 파일명 연도이고, 출력은 기존 `PublicationDates`다.
  발행·발간·간행·게재·인쇄 계열 마커와 날짜가 같은 줄에 있거나 줄 전체가
  날짜인 간기일 때만 `published_at`으로 확정한다. 마커처럼 보이는 본문 서술어
  `발행`·`발간`은 레이블로 인정하지 않고, 뒤쪽 머리말 간기보다 구조가 확인된
  표지 간기를 우선한다. 물리 1페이지도 KIDA Brief 헤더, 연구보고서 식별자,
  정기간행물 권·호와 간기 등 구조 신호가 없으면 `BODY`로 분류한다.
  extractor version은 `1.0.5`로 올렸고, `domain/` 계약과 의존성은 추가로
  변경하지 않았다. 변경 파일은 `search/metadata.py`, 대응 unit test와 이
  P1.5 기록이다.
- 실제 자료를 읽지 않는 deterministic fixture에서 Brief 물리 1페이지의
  본문 산문 날짜와 대시 부제 기간, 본문의 `발행` 서술어를 음성 경계로
  고정했다. 같은 줄의 발간 마커, 표지의 날짜-only 간기, 뒤쪽 페이지의
  기관장 서명 간기를 양성 경로로 검증했다. 완료 조건은 두 Brief 모양의
  `published_at=null`, `filename_year` 보존, 명시적 `failure_reason`, 양성 간기
  확정과 대응 검색 unit suite 통과다.
- 중복을 제거한 읽기 전용 corpus 370건 재측정 결과 확정률은 국방정책연구
  59/59(100.0%), 연구보고서 35/38(92.1%), 국방논단 0/100(0.0%), Brief
  0/173(0.0%), 전체 94/370(25.4%)였다. 국방정책연구는 논문 말미의
  `게재 확정: <날짜>` 줄로 전부 확정했다. 국방논단 100건의 머리줄은
  모두 `제…호(…) + <날짜>` 결합형으로 발행 마커가 없고 줄 전체가 날짜도
  아니므로 확정하지 않았다. Brief는 같은 근거 제한을 충족한 문서가
  없어 0건을 확정했다. 목표치보다 낮은 확정률을 높이기 위해 산문 날짜나 권·호 머리줄을
  발행일로 승격하지 않는다.
