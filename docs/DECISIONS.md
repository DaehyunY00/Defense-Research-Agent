...
## ADR-012 — 발행일은 유형별 간기 근거로만 확정한다

**Status: Accepted**

### Context

P1.5 초기 구현이 본문 산문의 날짜를 발행일로 승격했다. 실코퍼스 확인:

- `2022_김기범_…`: 본문 "◎ 2021년 1월, 새롭게 취임한 조 바이든 대통령은…"
  → `published_at=2021-01-01`. 바이든 취임 시점이지 발행일이 아니다.
- `2021_박대광_…`: 부제 "- 하노이 북미 정상회담 직후부터 2020년 6월까지를 대상으로 -"
  → `published_at=2020-06-01`. 연구 대상 기간이지 발행일이 아니다.

원인은 `COVER_PAGE` 근거를 `page_number == 1` 하나로 판정한 것이다. Brief 는 본문이 물리
1페이지에 인쇄되므로 본문 산문이 표지 근거 강도를 얻었다. Brief 는 코퍼스의 46% 다.

또한 국방정책연구 61/61 이 `게재 확정: YYYY년 M월 D일` 을 발행일로 쓰고 있었다. 이는
심사 통과일이며, 같은 호의 논문들이 서로 다른 발행일을 갖게 된다.

### Decision

유형별로 아래 근거에서만 `published_at` 을 확정한다. 그 외에는 `None` 과 명시적
`failure_reason` 을 남기고 `filename_year` 를 보존한다.

| 유형 | 근거 | precision | 확정 |
|---|---|---|---|
| 국방논단 | 간기 머리줄 `제N호(YY-MM) YYYY년 M월 D일` | day | 100/100 |
| 국방정책연구 | 머리줄 `국방정책연구 YYYY년 <계절>(권-호)` | season | 61/61 |
| 연구보고서 | 간기줄 | month | 35/38 |
| Brief | 없음 | — | 0/173 |

전체 196/372(53%). 나머지 47% 는 `filename_year` 가 유일한 연도 신호다.

`게재 확정` 날짜를 발행일로 쓰지 않는다. 산문 안의 날짜, 부제의 기간 표기, 저널 소개
문구(`연 4회 발행하는 계간지로서 2008년…`), 다른 발간물 언급은 모두 근거가 아니다.

`PublicationDates.failure_reason` 을 계약에 추가한다. 원 계약에는 날짜의 미확정 사유를
기록할 자리가 없어 "추측 대신 null 과 실패 사유" 규칙을 날짜에 적용할 수 없었다.

### Consequences

- 시계열 분석과 temporal backtest 는 코퍼스의 47% 에 `published_at` 이 없다는 전제 위에
  설계해야 한다. `filename_year` 가 대체 신호다.
- `DatePrecision.SEASON` 이 처음으로 실제 생성된다. 국방정책연구 61건이다.
- `DATA_QUALITY_REPORT.md:151` 이 국방논단 발행 표기 예로 든 값은 간기 머리줄에서
  회수된다. Brief 의 `2020. 7.` 은 현재 JSON 추출 텍스트에서 확인되지 않는다. 레인 E 가
  추가한 PDF 직접 추출로 회수되는지는 미확인이며 `PageTextSelection` 으로 비교할 수 있다.
- 확정률을 목표 지표로 삼지 않는다. 잘못된 구현도 54% 를 냈고 올바른 구현은 53% 다.
  숫자가 아니라 개별 근거가 판정 기준이다.
